from xy_sku_spider.dao.poi_city_dao import update_poi_city, query_poi_city_by_page
...
class PoiCityDataSpider(BaseSpider):
    ...
    def get_city_info(self, poi_city_id, url):
        ...
        try:
            ...
            if ul == None:
                loggings.info('爬取url:{0}--------------->未获取到数据 '.format(url))
                return
            lis = ul.find_all('li')
            ...
            loggings.info('爬取url:{0}--------------->end '.format(url))
        except Exception as e:
            loggings.error("************get_city_info error:{0}，url:{1}".format(e, url))
            write_str_to_dfile('error_poiCity_urls.txt', url)
            traceback.print_exc()

    def start(self, size):
        for index in range(1, size):
            loggings.info('爬取页码：{0}-----------------'.format(index))
            rows = query_poi_city_by_page(index, 10)
            threads = []
            for row in rows:
